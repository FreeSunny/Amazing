...
/**
 * https://github.com/nuptboyzhb/SuperSwipeRefreshLayout/blob/master/SuperSwipeRefreshLayout-Demo-AS/lib/src/main/java/com/github/nuptboyzhb/lib/SuperSwipeRefreshLayout.java
 */
public class SuperSwipeRefreshLayout extends ViewGroup {
    private static final String LOG_TAG = "SuperSwipeRefreshLayout";
    private static final int HEADER_VIEW_HEIGHT = 120;// HeaderView height (dp)
    private static final int FOOTER_VIEW_HEIGHT = 55;// HeaderView height (dp)
    ...
    private static final int DEFAULT_CIRCLE_TARGET = 90;
    ...
    private boolean mPullRefreshUI = false;
    private boolean mPullRefreshOutsize = false;
    private boolean mLoadMoreUI = false;
    private boolean mLoadMoreOutsize = false;
     ...
     */
    public void setPullRefreshUI(boolean refreshing) {
        if (refreshing && !mPullRefreshUI) {
            // scale and show
            mPullRefreshUI = true;
            mPullRefreshOutsize = true;
            int endTarget = 0;
            ...
            setPullRefreshUI(refreshing, false /* notify */);
            if (usingDefaultHeader) {
                defaultProgressView.setOnDraw(false);
            }
        }
    ...
    private void setPullRefreshUI(boolean refreshing, final boolean notify) {
        if (mPullRefreshUI != refreshing) {
            mNotify = notify;
            ensureTarget();
            mPullRefreshUI = refreshing;
            if (mPullRefreshUI) {
        ...
        if (!refreshing && notify){
            notifyPullRefreshEnd();
        }
    }
...
//        LogUtil.fish("mHeadViewContainer height " + mHeadViewContainer.getHeight());
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (!isEnabled() || mReturningToStart || isPullRefresh() || isLoadMore()
...
//                    }
            ...
            case MotionEvent.ACTION_CANCEL: {
                ...
                mIsBeingDragged = false;
                if (overscrollTop > mTotalDragDistance) {
                    setPullRefreshUI(true, true /* notify */);
                } else {
                    mPullRefreshUI = false;
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_CANCEL: {
                ...
                if (Build.VERSION.SDK_INT < Build.VERSION_CODES.HONEYCOMB) {
                    updateFooterViewPosition();
                    if (pushDistance == mFooterViewHeight && mOnPushLoadMoreListener != null && !isLoadMore()) {
                        mLoadMoreUI = true;
                        mLoadMoreOutsize = true;
    ...
    private void animatorFooterToBottom(int start, final int end) {
        ...
        valueAnimator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                if (end > 0 && mOnPushLoadMoreListener != null && !isLoadMore()) {
                    // start loading more
                    mLoadMoreUI = true;
                    mLoadMoreOutsize = true;
                    mOnPushLoadMoreListener.onLoadMore();
     ...
     * @param loadMore
     */
    public void setLoadMoreUI(boolean loadMore) {
        if (loadMore) {
            if (!mLoadMoreUI){
                mLoadMoreUI = true;
                mLoadMoreOutsize = true;
    ...
    private void notifyLoadMoreEnd(){
        if (mOnPushLoadMoreListener != null){
            mOnPushLoadMoreListener.onLoadMoreEnd(mLoadMoreUI, mLoadMoreOutsize);
        }
    }

    private void notifyPullRefreshEnd(){
        if (mOnPullRefreshListener != null){
            mOnPullRefreshListener.onRefreshEnd(mPullRefreshUI, mPullRefreshOutsize);
        }
    }
    ...
    public void stopLoading() {
        if (isLoadMore()) {
            //正在加载更多
            setLoadMoreUI(false);
            mLoadMoreOutsize = false;
            notifyLoadMoreEnd();
        } else if (isPullRefresh()) {
            //正在刷新
            setPullRefreshUI(false);
            mPullRefreshOutsize = false;
            notifyPullRefreshEnd();
        }
    }

    public boolean isLoadMore(){
        return mLoadMoreUI || mLoadMoreOutsize;
    }

    public boolean isPullRefresh(){
        return mPullRefreshUI || mPullRefreshOutsize;
    }
    ...
    private void setTargetOffsetTopAndBottom(int offset, boolean requiresUpdate) {
        ...
        mCurrentTargetOffsetTop = mHeadViewContainer.getTop();
        if (requiresUpdate && Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            invalidate();
        }
        if (targetScrollWithLayout) {
            ensureTarget();
    ...
    public interface OnPullRefreshListener {
        ...
        void onRefreshEnd(boolean uiRefreshing, boolean outsizeRefreshing);
    ...
    public interface OnPushLoadMoreListener {
        ...
        void onLoadMoreEnd(boolean uiRefreshing, boolean outsizeRefreshing);
    ...
    public class OnPullRefreshListenerAdapter implements OnPullRefreshListener {
        ...
        @Override
        public void onRefreshEnd(boolean uiRefreshing, boolean outsizeRefreshing) {
    ...
    public class OnPushLoadMoreListenerAdapter implements OnPushLoadMoreListener {
        ...
        @Override
        public void onLoadMoreEnd(boolean uiRefreshing, boolean outsizeRefreshing) {
